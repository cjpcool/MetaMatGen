...
def get_pbc_distances(
    ...
    offsets = torch.einsum('bi,bij->bj', to_jimages.to(torch.float), lattice_edges)
...
def calculate_to_jimages_efficient(coordinates, edge_index, lattice_vectors, num_bonds=None):
    """
    ...
    if isinstance(lattice_vectors, torch.Tensor):
        lattice_vectors = lattice_vectors.cpu().numpy()

    if isinstance(num_bonds, torch.Tensor):
        num_bonds = num_bonds.cpu().numpy()
    ...
    if num_bonds is not None:
        lattice_inv = np.repeat(lattice_inv, num_bonds, axis=0)
        lattice_vectors = np.repeat(lattice_vectors, num_bonds, axis=0)
    # 对于每条边
    for idx, (i, j) in enumerate(edge_index.T):
        vector_ij = coordinates[j] - coordinates[i]  # 原始距离向量
        jimages, corrected_vector = find_images_and_correct_vector(vector_ij, lattice_inv[idx], lattice_vectors[idx])
